...
import { FeatureName } from '../common/hopcoderSettingsTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IhopcoderSettingsService private readonly _settingsService: IhopcoderSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@IhopcoderModelService private readonly _hopcoderModelService: IhopcoderModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._hopcoderModelService.initializeModel(model.uri)
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._hopcoderModelService.getModel(uri)
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const onFinishEdit = async () => {
			...
			await this._hopcoderModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._hopcoderModelService.initializeModel(uri)
		await this._hopcoderModelService.saveModel(uri) // save the URI
	}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IhopcoderCommandBarService private readonly _hopcoderCommandBarService: IhopcoderCommandBarService,
		...
		const getAcceptRejectText = () => {
			...
			const commandBarStateAtUri = this._hopcoderCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._hopcoderCommandBarService.onDidChangeState(e => {
