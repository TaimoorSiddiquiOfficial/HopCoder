...
const performVoidCheck = async (
	...
	hopcoderUpdateService: IhopcoderUpdateService,
	...
	const res = await hopcoderUpdateService.check(explicit)
	if (!res) {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const hopcoderUpdateService = accessor.get(IhopcoderUpdateService)
		...
		const newController = await performVoidCheck(true, notifService, hopcoderUpdateService, metricsService, updateService)
...
class VoidUpdateWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		@IhopcoderUpdateService hopcoderUpdateService: IhopcoderUpdateService,
		...
		const autoCheck = () => {
			performVoidCheck(false, notifService, hopcoderUpdateService, metricsService, updateService)
		}
