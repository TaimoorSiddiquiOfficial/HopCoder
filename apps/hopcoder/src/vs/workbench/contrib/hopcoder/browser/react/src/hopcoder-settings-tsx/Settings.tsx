...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('IhopcoderSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/TaimoorSiddiquiOfficial/HopCoder/blob/main/src/vs/workbench/contrib/hopcoder/common/modelCapabilities.ts#L146-L172`
		...
		>
			...
			>
				...
				<div className="text-sm text-void-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with HopCoder, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by HopCoder.`
							: `HopCoder recognizes ${modelName} ("${recognizedModelName}").`}
				</div>
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='hopcoder-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='hopcoder-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
					...
					{disabled ? null : (
						<div className="w-5 flex items-center justify-center">
							<button
								...
								data-tooltip-id='hopcoder-tooltip'
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const accessor = useAccessor()
	const hopcoderSettingsService = accessor.get('IhopcoderSettingsService')
	const settingsState = useSettingsState()
	...
	const handleChangeValue = useCallback((newVal: string) => {
		hopcoderSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [hopcoderSettingsService, providerName, settingName]);
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = hopcoderSettingsService.state.settingsOfProvider[providerName].enabled
// 						hopcoderSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [hopcoderSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	return <ButtonLeftTextRightOption
		leftButton={<VoidSwitch
			...
			value={enabled}
			onChange={(newVal) => {
				hopcoderSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	return <VoidInputBox2
		...
		onChangeText={(newText) => {
			hopcoderSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		hopcoderSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [hopcoderSettingsService])

	return <VoidCustomDropdownBox
		className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={hopcoderSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		<div
			...
			data-tooltip-id='hopcoder-tooltip-ollama-settings'
		>
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	return <div
		...
		onClick={() => { hopcoderSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		hopcoderSettingsService.setGlobalSetting('autoApprove', {
			...hopcoderSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [hopcoderSettingsService, metricsService])
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'hopcoder-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(hopcoderSettingsService.state, null, 2)
			downloadName = 'hopcoder-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					hopcoderSettingsService.dangerousSetState(json as any)
				}
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`HopCoder's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`HopCoder can access any model that you host locally. We automatically detect your local models by default.`}</h3>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`HopCoder can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
								...
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<VoidSwitch
																...
																onChange={(newVal) => hopcoderSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => hopcoderSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => hopcoderSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => hopcoderSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of HopCoder suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => hopcoderSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => hopcoderSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-void-fg-3 mb-4'>{`Transfer your editor settings into HopCoder.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Transfer HopCoder's settings and chats in and out of HopCoder.`}</h4>
									<div className='flex flex-col gap-8'>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { hopcoderSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-void-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep HopCoder running smoothly. You may opt out below. Regardless of this setting, HopCoder never sees your code, messages, or API keys.</h4>
...
Alternatively, place a \`.hopcoderrules\` file in the root of your workspace.
								`} chatMessageLocation={undefined} />
									...
									<div className='my-4'>
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<VoidSwitch
													...
													onChange={(newValue) => {
														hopcoderSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
										...
										<div className='text-void-fg-3 text-xs mt-1'>
											{`When disabled, HopCoder will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
