...
import { mountVoidCommandBar } from './react/out/hopcoder-editor-widgets-tsx/index.js'
...
export interface IhopcoderCommandBarService {
...
export const IhopcoderCommandBarService = createDecorator<IhopcoderCommandBarService>('hopcoderCommandBarService');
...
export class hopcoderCommandBarService extends Disposable implements IhopcoderCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'void.hopcoderCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._hopcoderModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IhopcoderCommandBarService, hopcoderCommandBarService, InstantiationType.Delayed); // delayed is needed here :(
