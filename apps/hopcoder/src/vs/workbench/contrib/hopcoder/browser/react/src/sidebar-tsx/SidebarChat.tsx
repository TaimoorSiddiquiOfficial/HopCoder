...
import { ModelDropdown, } from '../hopcoder-settings-tsx/ModelDropdown.js';
import { PastThreadsList } from './SidebarThreadSelector.js';
import { VOID_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { VOID_OPEN_SETTINGS_ACTION_ID } from '../../../hopcoderSettingsPane.js';
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/void/common/hopcoderSettingsTypes.js';
import { ICommandService } from '../../../../../../../platform/commands/common/commands.js';
import { WarningBox } from '../hopcoder-settings-tsx/WarningBox.js';
...
import { ToolApprovalTypeSwitch } from '../hopcoder-settings-tsx/Settings.js';
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSwitch
				...
				onChange={(newVal) => {
					...
					hopcoderSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff })
				}}
			/>
		</div>
	}
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSlider
				...
				onChange={(newVal) => {
					...
					hopcoderSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal })
				}}
			/>
	...
	if (reasoningBudgetSlider?.type === 'effort_slider') {
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSlider
				...
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && newVal === valueIfOff
					hopcoderSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined })
				}}
			/>
...
const ChatModeDropdown = ({ className }: { className: string }) => {
	...
	const onChangeOption = useCallback((newVal: ChatMode) => {
		hopcoderSettingsService.setGlobalSetting('chatMode', newVal)
...
export const SelectedFiles = (
	...
	const accessor = useAccessor()
	const commandService = accessor.get('ICommandService')
	const modelReferenceService = accessor.get('IhopcoderModelService')
...
export const SidebarChat = () => {
	...
	const initiallySuggestedPromptsHTML = <div className='flex flex-col gap-2 w-full text-nowrap text-void-fg-3 select-none'>
		{[
			...
			'Create a .hopcoderrules file for me'
		].map((text, index) => (
			<div
				key={index}
