...
import { MCPUserStateOfName } from './hopcoderSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	private async _initialize() {
		try {
			await this.hopcoderSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.hopcoderSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.hopcoderSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.hopcoderSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.hopcoderSettingsService.setMCPServerState(serverName, { isOn });
